...
def login():
    ...
    if user and user.password == password:  # Placeholder logic, add password hashing comparison
        return redirect(url_for('welcome', username=user.first_name))  # Passing username for the welcome route
    else:
...
def welcome():
    ...
    return render_template('homepage_update.html', content=content)
